...
const HeaderChat = props => {
  ...
  const fillNameGroup = useCallback(() => {
    if (group) return currentGroup.name;
    if (!group) {
      let name = users
        ? users.filter(user => user.id !== dataUser.id)[0].name
        : currentGroup.name;
      return name;
    }
  }, [currentGroup.name, dataUser.id, group, users]);
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row' }}>
        ...
        <View>
          <View style={{ flexDirection: 'row' }}>
            <Text style={styles.name}>{fillNameGroup()}</Text>
            {group && (
              <TouchableOpacity
